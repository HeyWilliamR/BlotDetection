...
class VGG16 :
    ...
    def conv_op(self,input_op,name,kh,kw,n_out,dh,dw):
        ...
        with tf.name_scope(name) as scope:
            kernel = tf.get_variable(scope+"_W",shape = [kh,kw,n_in,n_out],dtype = tf.float32,
                                     initializer=tf.contrib.layers.xavier_initializer_conv2d())
            conv = tf.nn.conv2d(input_op,kernel,(1,dh,dw,1),padding="SAME")
            bias_init_val = tf.constant(0.0,shape=[n_out],dtype=tf.float32)
            biases = tf.Variable(bias_init_val,trainable=True,name = "_b")
    ...
    def fc_op(self, input_op, name, n_out):
        ...
        with tf.name_scope(name) as scope:
            kernel = tf.get_variable(scope+"_W",shape=[size,n_out],dtype = tf.float32,
                                     initializer= tf.contrib.layers.xavier_initializer())
            biases = tf.Variable(tf.constant(0.1,shape=[n_out],dtype=tf.float32),name = "_b")
    ...
    def fc_layers(self):
        self.fc6 = self.fc_op(input_op=self.pool5,name = 'fc6',n_out=2048)
        ...
        self.fc8 = self.fc_op(input_op=self.fc7_drop,name="fc8",n_out = 4)
    def saver(self):
